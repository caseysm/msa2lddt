...
def evaluate_alignment_quality(pdb_file, fasta_file, max_rmsd):
    # Parse PDB file
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate alignment quality using backbone RMSD and similarity scores")
    parser.add_argument("pdb_file", help="Path to the PDB file")
    parser.add_argument("fasta_file", help="Path to the FASTA alignment file")
    parser.add_argument("--max_rmsd", type=float, default=10.0,
                        help="Maximum RMSD for similarity calculation (default: 10.0)")
    ...
    try:
        ...
        output_lines = [
            ...
            f"Max RMSD for similarity calculation: {args.max_rmsd} Å\n",
            "Residue-by-residue backbone RMSD and similarity scores:\n"
        ]
        ...
        overall_similarity = np.mean([result['similarity'] for result in alignment_quality])
        output_lines.append(f"\nOverall Alignment RMSD: {overall_rmsd:.4f} Å\n")
        output_lines.append(f"Overall Alignment Similarity: {overall_similarity:.4f}\n")
